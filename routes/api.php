...
Route::get('mailer', function(){
    ...
    // ];
       ...
       $data = [
         ...
         //validateUser
      \Mail::send('backend.mails.welcome', $data, function ($message) use ($data) {
  ...
  Route::get('download/certificates/{id}', 'CertificateController@certificado_download_pdf');
  ...
  Route::get('/activate', function(){ return view('auth.activate'); });
  ...
  Route::prefix('backend')->group(function () {
    Route::get('/', function () {  return  \Redirect::action( 'HomeController@index');  });
    Route::get('home', 'HomeController@index')->name('home');
    Route::resource('usuarios','UserAllController', ['names' => ['store' => 'user.create']]);
    Route::resource('empresa','CompanyController', ['names' => ['store' => 'company.store', 'update' => 'company.update', 'destroy' => 'company.destroy']]);
    ...
    Route::get('theme', 'UserAllController@changeTheme');
      ...
      Route::resource('post','PostController', ['names' => ['store' => 'post.store', 'update' => 'post.update']]);
      Route::resource('opciones','OptionsController', ['names' => ['store' => 'option.store', 'update' => 'option.update']]);
      // Files | Multimedia
      
      Route::resource('menu', 'LinksController', ['names' => ['store' => 'menu.store', 'update' => 'menu.update', 'destroy' => 'menu.destroy']]);
      Route::resource('taxonomias', 'TaxonomiesController', ['names' => ['store' => 'taxonomy.store', 'update' => 'taxonomy.update', 'destroy' => 'taxonomy.destroy']]);
      Route::get('taxonoable', 'TaxonomiesController@taxonoable');
      Route::resource('laboratorios', 'LaboratoriesController', ['names' => ['store' => 'lab.store', 'update' => 'lab.update', 'destroy' => 'lab.destroy']]);
  
      // Curso 
      Route::resource('curso','CoursesController', ['names' => ['index' => 'course.index', 'store' => 'course.store', 'update' => 'course.update']]);
      ...
      Route::delete('curso/validacion/delete/{id}','CoursesController@validateCourseDelete');
      Route::post('curso/ratting/modify/{id}','CoursesController@rattingCourse');
      Route::post('curso/view/{id}','CoursesController@viewsCourse');
      // Contenido 
      Route::resource('contenido','ContentsController', ['names' => ['index' => 'content.index', 'store' => 'content.store', 'update' => 'content.update']]);
      Route::get('contenido/lista/{id}','ContentsController@lista');
      // Actividad
      Route::resource('actividad','InteractionController', ['names' => ['store' => 'interaction.store', 'update' => 'interaction.update']]);
      Route::post('actividad/inicial/interaccion','InteractionController@initialInteraction');
      // Generar interacciones 
      Route::put('interactiontext','InteractionController@interactiontext');
      Route::put('interactionChargeFile','InteractionController@interactionChargeFile');
      Route::put('interactionQuestionMultiple','InteractionController@interactionQuestionMultiple');
      Route::put('interactionQuestion','InteractionController@interactionQuestion');
      Route::put('interactionFileAudio','InteractionController@interactionFileAudio');
      ...
      Route::delete('certificado/{id}', 'CertificateController@certificate_destroy');
      
      Route::get('search/certificates/', 'CertificateController@certificado_search');
      Route::get('search/certificates/result', 'CertificateController@certificado_search_result');  
      Route::get('certificado/curso/{id}', 'CertificateController@register_certificate');
      ...
      Route::prefix('roles')->group(function () {
        ...
        Route::get('list/{id}', 'RolesController@roles_store')->name('admin.store.roles');
        Route::get('permisos', 'RolesController@permission')->name('admin.permission');
        Route::post('permisos', 'RolesController@permission_store')->name('admin.permission.store');
        Route::post('asignar', 'RolesController@assign')->name('admin.assign.store');
      });
  
      //FILES
      Route::get('fileable', 'MultimediaController@fileable');
      Route::get('fileable/destroy', 'MultimediaController@fileable_destroy');
      Route::resource('archivos', 'MultimediaController', ['names' => [ 'store' => 'file.store', 'destroy' => 'file.destroy', 'update' => 'file.update', 'edit' => 'file.edit' ] ]);
  });
