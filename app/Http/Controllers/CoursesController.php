...
class CoursesController extends Controller
{
    ...
    {
        $this->middleware(['permission:course.update'])->only(['update', 'edit']);
        $this->middleware(['permission:course.create'])->only(['create', 'store']);
        $this->middleware(['permission:course.list'])->only(['index']);
        $this->middleware(['permission:course.destroy'])->only(['destroy']);
        $this->middleware(['permission:course.show'])->only(['show', 'courseList']);
        $this->middleware(['permission:enroll.update'])->only(['validateCourse']);
    ...
    {
        $array = ['normal', 'masterClass', 'course', 'tutorial', 'review', 'audit', 'webinar', 'seminary', 'conference', 'webcast', 'meeting', 'reading', 'mooc', 'spoc', 'poadcast', 'video', 'smallTalk'];
    ...
    public function index(Request $request)
    {
        ...
        if (count($courses) > 0) {
            foreach ($courses as $key => $course) {
                $person = Data::where('user_id', $course->subject)->select('id', 'first', 'last')->first();
        ...
        return view('backend.course.list', compact('type', 'courses', 'users', 'contents'));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('backend.course.create', compact('type', 'listFormats'));
    }
    ...
    {       
        ...
        Courses::create($make);
        Session::flash('snackbar-success', 'Curso creado correctamente');
        return back();
    }

    public function add_img()
    {
        $course = Courses::find(1)->files()->create(['files_id' => 1]);
        Session::flash('snackbar-success', 'Curso imagen creada correctamente');
        return $course;
    }
    ...
    public function edit($id, Request $request)
    {
        ...
        $course = Courses::where('id', $id)->where('context', $type)->first();

        return view('backend.course.update', compact('type', 'course', 'taxonomies'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Courses::where('id', $id)->update($make);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        if ($force == 1) {
            $options->forceDelete();
            Session::flash('snackbar-warning', 'El curso se ha Eliminado totalmente');
            return back();
        }
        $options->delete();
        Session::flash('snackbar-warning', 'El curso se ha envíado a la papelera');
        return back();
    }
    ...
    {
        $courses = Courses::all();      
        return view('backend/course/first-list-courses', compact('courses'));
    }

    // muestra el curso en especifico y permite registrarse
    public function show(Request $request, $id)
    {   
        $course = Courses::where('slug', $id)->orWhere('id', $id)->first();
        if ($course == null) {
            abort(404);
        }
        $cdt = Carbon::now();
        ...
        $ifUserRegister = DB::table('course_user')->where('user_id', Auth::user()->id)
            ->where('courses_id', $course->id)->get();
        ...
        $contents = DB::table('course_configurate')
            ->leftJoin('contents', 'contents_id', '=', 'contents.id')
            ->select('course_configurate.id', 'contents_id', 'contents.content', 'contents.timeline', 'contents.slug', 'contents.type')
            ->orderBy('contents.order', 'asc');
        
        // Evalua el status actual del contenido 
        if( !in_array('admin', Auth::user()->getRoleNames()->toArray()) ) {
            $contents = $contents->where('contents.status', 1)->where('courses_id', $course->id)->get();
        } else {
        ...
        if($course->timeOut != null && $course->created_at != null){
            ...
            if( $created_at->equalTo($timeOut) ){
                $timerOut = 0; 
            } else if ( $timeActual->greaterThan($timeOut) ) {
                $timerOut = 1; 
            } 
        }

        return view('backend/course/courses-find', compact('course', 'registered', 'reviewed', 'timerOut', 'contents', 'finalTime'));
    }
    ...
    public function register(Request $request)
    {
        if (Auth::check()) {
            ...
            $validate = DB::table('course_user')->where('user_id', Auth::user()->id)
            ->where('courses_id', $request->course_id)->get();
    ...
    public function matriculados(Request $request)
    {
        $usuarios = DB::table('course_user')
                    ...
                    ->select('course_user.id as id', 'users.id as user_id', 'users.name', 'courses.id as course_id','courses.course', 'course_user.status as status_course')
                    ->orderBy('courses.course', 'asc')
                    ->get();
        return view('backend.course.matricula', compact('usuarios'));
    }
    ...
    public function validateCourse($id)
    {
        ...
        Session::flash('snackbar-success', 'Usuario Matriculado correctamente');
        return back();
    }
    ...
    public function validateCourseDelete(Request $request, $id)
    {
        ...
        Session::flash('snackbar-danger', 'Usuario Eliminado correctamente');
        return back();
    }

    public function rattingCourse($id, Request $request){

        $realizado = State::where('stateable', $request->course_id)->where('user_id', $id)->get();
        $course = Courses::where('id', $request->course_id)->update([
            'rating' => (int)State::where('key', 'rating_course')->avg('value')
        ]);
        ...
        $state = [
            ...
            'value' => $request->value,
            'description' => 'Curso: ' . $request->course_id . ' Calificación: ' . $request->value . ' ' .$request->description,
            'status' => 1,
            'user_id' => $id
        ];
        $result = null; 
        ...
        if($result == 'ok'){
            return ['msj' => 'ok', 'url' => '/backend/notas/certificacion/'. $id .'?course='. $request->course_id  .'&calificar=0']; 
        } else {
            return ['msj' => 'La calificación ha fallado porfavor intentelo más tarde'];
        }
    }
