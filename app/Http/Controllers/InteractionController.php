...
class InteractionController extends Controller
{
    ...
    {
        $this->middleware(['permission:interaction.update'])->only(['update', 'edit']);
        $this->middleware(['permission:interaction.create'])->only(['create', 'store']);
        $this->middleware(['permission:interaction.list'])->only(['index']);
        $this->middleware(['permission:interaction.destroy'])->only(['destroy']);
        $this->middleware('auth');
    }

    public function valArray($type)
    {
        $array = ['ver:todo', 'notas', 'seleccion:multiple', 'calificaciones', 'respuesta:text', 'cargar:archivo', 'cargar:imagen', 'escuchar:audio', 'ver:video', 'leer', 'close', 'asistir:reunion', 'descargar'];
    ...
    public function initialInteraction(Request $request){
        if ($this->valArray($request->type) == 1) {
            $make = $request->all();
            $revision = DB::table('configurates')->where('courses_id', $make['course_id'])
            ->where('contents_id', $make['content_id'])
            ->where('user_id', Auth::user()->id);
            if($request->interactions_id != null){
                $revision = $revision->where('interactions_id', $request->interactions_id)->get();
            } else {
            ...
            if( count($revision) == 0 ){
                ...
                Interaction::where('id', $interactionNew->id)->update([
                    ...
                    'slug' => str_slug($interactionNew->id ."-".$content->slug."-".$request->type, '-'),
                    'status' => 1,
                    'type' => $request->type
                ]);
                DB::table('configurates')->insert([
                    ...
                    'status' => 1,
                    'requiredActivities' => ($make['assing'] == null) ? null : 'close|'.$make['assing'], 
    ...
    public function interactionQuestion(Request $request){
        if ($this->valArray($request->type) == 1) {
            ...
            if($make['response']){  
                ...
                $configurates = DB::table('configurates')->where('interactions_id', $make['interaction_id'])
                ...
                $state = [
                    ...
                    'description' => 'Respuesta a pregunta de la actividad '. $configurates[0]->id,
                    'status' => 1,
                ...
                DB::table('configurates')->where('id', $configurates[0]->id)->update([
        ...
        if ($this->valArray($request->type) == 1) {
            ...
            if($make['response'] && $make['value']){
                ...
                $state = [
                    ...
                    'description' => 'Respuesta a pregunta multiple en la actividad '. $configurates[0]->id,
                    'status' => 1,
                    'stateable' => $configurates[0]->id,
                    'user_id' => Auth::user()->id
                ];
    ...
    public function interactionText (Request $request){
        if ($this->valArray($request->type) == 1) {
            ...
            if($make['content']){
                ...
                $state = [
                    'key' => 'interactionText',
                    'description' => 'Vinculación de información a la actividad '. $configurates[0]->id,
                    'status' => 1,
                    'stateable' => $configurates[0]->id,
                    'user_id' => Auth::user()->id
                ];
        ...
        if ($this->valArray($request->type) == 1) {
            ...
            $selection = implode("|", $selection);
                ...
                $state = [
                    ...
                    'description' => 'Cierre de la actividad '. $make['content_id'],
                    'status' => 1,
                    'stateable' => $make['content_id'],
                    'user_id' => Auth::user()->id
                ];
    ...
    {
        $interaction = Interaction::find(1)->files()->create(['files_id' => 1]);
        Session::flash('snackbar-success', 'Contenido creado correctamente');
        return $interaction;
    }
    ...
    public function edit($id, Request $request)
    {
        ...
        $interactions = Interaction::where('id', $id)->where('type', $type)->first();
        return view('backend.interaction.update', compact('type', 'interactions', 'taxonomies'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Interaction::where('id', $id)->update($make);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        if ($force == 1) {
            $options->forceDelete();
            Session::flash('snackbar-warning', 'El contenido se ha Eliminado totalmente');
            return back();
        }
        $options->delete();
        Session::flash('snackbar-warning', 'El contenido se ha envíado a la papelera');
        return back();
    }
}