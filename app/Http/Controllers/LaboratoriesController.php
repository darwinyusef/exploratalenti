...
class LaboratoriesController extends Controller
{
    ...
    {
        // view: backend.laboratory.create
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            Session::flash(
                'snackbar-success',
                'Se a creado un nuevo Laboratorio'
            );
            return Notify::ms(
                'ok',
                201,
    ...
    {
        // view: .backend.laboratory.update
        $laboratory = Laboratories::findOrFail($id);
        return Notify::ms('ok', 201, $laboratory, 'Se a listado correctamente');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($data, Laboratories::$rules);
        if ($validator->fails()) {
            //backend/laboratorios/$id/edit
            return Notify::ms('error', 400, $validator, 'Validations Error');
        } else {
            // se carga la solicitud
            $final = Laboratories::where('id', $id)->update($data);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        if ($force == 1) {
            $group->forceDelete();
            // return Redirect::to('/backend/laboratorios');
            return Notify::ms('ok', 201, [], 'Se a Elimninado el Registro');
        }
        $group->delete();
    ...
    public function moodle(Request $request)
    {
        ...
        if (isset($user)) {
            ...
            $obj = collect($obj)->filter(function ($value, $key) use (
                $empresa
            ) {
        ...
        return view(
            'backend.laboratory.new_users',
            compact('obj', 'count', 'company')
        );
    }
    ...
    public function list_mail_moodle(Request $request)
    {
        ...
        $out = $request->out;
        $url =
            'http://diagnosticar.com.co/moodle/register/json_transfer.php?in=1&out=' .
            $out;
        $json = file_get_contents($url);
        $obj = json_decode($json);
        ...
        $resultado = array_diff(
            $em,
            array_diff(
                array_unique($em),
                array_diff_assoc($em, array_unique($em))
            )
        );
        $resultadoid = array_diff(
            $ident,
            array_diff(
                array_unique($ident),
                array_diff_assoc($ident, array_unique($ident))
            )
        );

        return view(
            'backend.laboratory.list_duplicate',
            compact('resultado', 'resultadoid')
        );
    }
    ...
    public function chargue_moodle(Request $request)
    {
        ...
        $out = $request->out;
        $url =
            'http://diagnosticar.com.co/moodle/register/json_transfer.php?in=' .
            $in .
            '&out=' .
            $out;
        $json = file_get_contents($url);
        ...
        foreach ($obj as $usernew) {
            ...
            if (!in_array($usernew->email, $data)) {
                ...
                $tipo = null;
                $identify = trim(
                    preg_replace('/[^A-Za-z0-9 ]/', '', $usernew->idnumber),
                    ' '
                );
                if ($identify != '' && is_numeric($identify)) {
                    $usernew->idnumber = $identify;
                    $name =
                        ucwords($usernew->firstname) .
                        ' ' .
                        ucwords($usernew->lastname);
                    $slug = str_slug($name, '-');
                    ...
                    $iduser = Data::where('card_id', $identify)
                        ->select('user_id', 'id')
                        ->first();
                    if ($iduser != null) {
                        $usert = User::where('id', $iduser->user_id)->update(
                            $newUser
                        );
    ...
    public function moodle_company()
    {
        ...
        return view(
            'backend.laboratory.certificates_list',
    ...
    public function moodle_search_result(Request $request)
    {
        ...
        if (count($certificates) == 0) {
            return Notify::ms('no-found', 404, $validator, 'No existe el objeto');
        }
    ...
    public function moodle_download_pdf($id)
    {
        ...
        $course = Data::where('id_usermoodle', $data->iduser)
            ->select('course_id')
            ->first();

        $url =
            'http://diagnosticar.com.co/moodle/register/get_courses_laravel.php';
        $json = file_get_contents($url);
        $obj = json_decode($json);
        ...
        if ($course == null) {
            Session::flash(
                'snackbar-warning',
                'Error en la selección del curso del usuario: por favor modifique el curso del usuario'
            );
        ...
        $pdf = \PDF::loadView('backend.laboratory.certifica', $data)->setPaper(
            'a4',
            'landscape'
        );
    ...
    public function moodle_view($id)
    {
        ...
        if ($course == null) {
            Session::flash(
                'snackbar-warning',
                'Error en la selección del usuario'
            );
            return redirect('/search/certificates');
        }
        $url =
        ...
        return view(
            'backend.laboratory.certifica_view',
            compact('data', 'total')
        );
    }
    ...
    public function modify_id()
    {
        ...
        foreach ($emails as $email) {
            ...
            $certificate[] = Certificate::where('email', $id['email'])
                ->where('cedula', $id['card_id'])
                ->update($data);
        }
