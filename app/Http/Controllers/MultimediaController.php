...
use App\Models\Files as Multimedia;
...
class MultimediaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Auth::user()->hasAnyPermission('file.list')) {
            $files = DB::table('fileables')
                ->where('fileable_type', '<>', 'App\Models\User')
                ->leftJoin('multimedia', 'multimedia.id', '=', 'files_id')
                ->select(
        ...
        if (isset($request)) {
            $files = Multimedia::where(
                'name',
                'LIKE',
                '%' . $request->file . '%'
            )->paginate($paginate);
        }
        // view: backend.admin.files
        return Notify::ms('ok', 201, $files, 'Se a listado correctamente');
    }
    ...
    public function store(Request $request)
    {
        ...
        Session::flash('snackbar-success', '');
        return Notify::ms('ok', 201, [], 'Archivo creado correctamente');
    }

    public function edit($id)
    {
        // view: backend.admin.files_edit
    ...
    public function update(Request $request, $id)
    {
        ...
        if (empty($request->file($input_name))) {
            ...
            $archivo = [
                ...
                'type_file' => $file->getClientOriginalExtension(),
                'file_location' =>
                    env('APP_URL') . '/files/' . 'ac' . $file_name,
    ...
    public function fileable(Request $request, $consult = null)
    {
        ...
        if ($msj == 'success') {
            Session::flash(
                'snackbar-success',
                'El Archivo se a asignado correctamente'
            );
            return back();
        }
        if ($msj == 'parameter') {
            Session::flash(
                'snackbar-danger',
                'No se cuenta con todos los paramentros requeridos'
            );
            return back();
        }
        if ($msj == 'duplicate') {
            Session::flash(
                'snackbar-danger',
                'No se puede cargar un archivo duplicado'
            );
            return back();
        }
    }
