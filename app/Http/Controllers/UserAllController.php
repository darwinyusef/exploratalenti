...
class UserAllController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    $users = User::leftJoin('data_users', 'user_id', "=", "users.id")
      ...
      ->select('first', 'last', 'email', 'company', 'users.id as id', 'data_users.company_id', 'data_users.id as id_data',  'data_users.mobile', 'data_users.address', 'type_card', 'card_id', 'data_users.neighborhood');
  ...
  {
    $company = Company::all();
    return view('backend.user.user-create', compact('company'));
  }
  ...
  public function store(Request $request)
  {
    ...
    if ($validator->fails()) {
      ...
      $data = [
        ...
        'url' => env('APP_URL').'/verifyemail?type=&token='.$user->validate_token.'&email='.$solicitud['email'].'&live='.$user->id
      ];
  ...
  public function edit($id)
  {
    if (Auth::user()->hasRole('admin')) {
      $id = $id;
    } else if (Auth::user()->hasPermissionTo('user.my')) {
      $id = Auth::user()->id;
    }
    $user = Data::where('user_id', $id)->first();
    $profile = User::find($user->user_id);
    $company = Company::all();
    return view('backend.user.profile-update', compact('user', 'profile', 'company'));
  }
  ...
  public function show($id)
  {
    ...
    return view('backend.user.profile-show', compact('user', 'profile', 'company'));
  }
  ...
  public function update($id, Request $request)
  {
    ...
    if ($validator->fails()) {
      ...
      $institution = $request->company_id . ";" . Company::where('id', $request->company_id)->first()->company;
      ...
      $url = 'https://diagnosticar.com.co/moodle/register/get_update_laravel.php';
      $ch = curl_init($url);
      ...
      if ($rest->response == "ok") {
        ...
        $users = array_only($solicitud, ['email', 'name', 'active', 'slug', 'pago', 'status', 'validate_token']);
        if (Auth::user()->hasRole('admin')) {
          $id = $id;
        } else if (Auth::user()->hasPermissionTo('user.my')) {
          $id = Auth::user()->id;
        ...
        $datas = array_only($solicitud, [
          'first', 'last', 'type_card', 'card_id', 'postcode', 'gender',
          'mobile', 'phone_home', 'address', 'neighborhood', 'company_id'
        ]);
        $data = Data::where('user_id', $user->id)->update($datas);
        Session::flash('snackbar-success', 'Usuario Actualizado Correctamente: ' . $user->email);
        return redirect('/backend/usuarios/' . $id . '/edit');
      }
    }
  ...
  public function pass(Request $request)
  {
    ...
    return view('backend.user.profile-password-change', compact('id'));
  }
  ...
  public function pass_store(Request $request)
  {
    ...
    if ($validator->fails()) {
      ...
      Mail::send('backend.mails.changePass', $data, function ($message) use ($data) {
        $message->subject('Verifique su Email - Notification');
        $message->from('no-reply@aquicreamos.com');
        $message->to($data['email']);
      });
...
/// Mail
  public function restaurar(Request $request)
  {
    ...
    if ($user > 0 && $data > 0) {
      $changue = User::leftJoin('data_users', 'user_id', "=", "users.id")->where('email', $email)->where('card_id', $card_id)->select('users.id as id')->first();
      User::find($changue->id)->update([
        'password' => bcrypt('LabDiagnosticar@2018'),
      ]);
    }
    Session::flash('snackbar-success', 'Contraseña modificada Correctamente');
    return back();
  }
  }
